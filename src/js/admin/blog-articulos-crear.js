/* blog-articulos-crear
   Migrado desde el <script> embebido de views/blog/articulos/crear.php */
(function () {
    if (!document.body || document.body.dataset.page !== 'blog-articulos-crear') return;
    (function () {
        ...
        slugToggleBtn.addEventListener('click', function () {
            ...
            slugHidden.disabled = slugManualMode; // evita duplicado en POST
            if (slugManualMode) {
                slugManual.value = slugPreview.textContent;
                slugManual.focus();
            }
        });
        ...
        function actualizarSEO() {
            ...
            if (len === 0) {
                ...
                label = `${len} / 40 · Título muy corto para SEO`; cls = 'seo--warn';
            } else if (len <= 40) {
                label = `${len} / 40 · ¡Longitud perfecta para Google!`; cls = 'seo--good';
            } else if (len <= 55) {
                label = `${len} / 40 · Algo largo, considera recortarlo`; cls = 'seo--warn';
            } else {
                label = `${len} / 40 · Demasiado largo para Google`; cls = 'seo--bad';
            }
        ...
        extracto.addEventListener('input', function () {
            extractoCount.textContent = this.value.length;
        });
        ...
        // Guardar selección antes de clic en toolbar (el foco sale del editor)
        ...
        function renderizarTags() {
            ...
            tags.forEach((tag, i) => {
                ...
                pill.innerHTML = escHtml(tag) +
                    '<button type="button" class="tag-pill__remove" data-i="' + i + '" title="Quitar etiqueta">' +
        ...
        tagWrap.addEventListener('click', e => {
            if (e.target.closest('.tag-pill__remove')) {
                const i = parseInt(e.target.closest('[data-i]').dataset.i);
                tags.splice(i, 1);
                renderizarTags();
            } else {
                tagInput.focus();
        ...
        tagInput.addEventListener('input', function () {
            if (this.value.includes(',')) {
                ...
                partes.slice(0, -1).forEach(p => agregarTag(p));
                this.value = partes[partes.length - 1];
            }
        });
        ...
        imagenInput.addEventListener('change', () => mostrarPreview(imagenInput.files[0]));

        imageZone.addEventListener('dragover', e => {
            e.preventDefault();
            imageZone.classList.add('image-upload-zone--over');
        });
        ...
        /* ─────────────────────────────────────────────────────
           10. SUBMIT — SINCRONIZAR WYSIWYG AL HIDDEN INPUT
        ───────────────────────────────────────────────────── */
        document.getElementById('articuloForm').addEventListener('submit', function () {
            contenidoHid.value = editor.innerHTML;
            // Si el slug manual está activo, deshabilitar el hidden para no duplicar
            if (slugManualMode) {
                slugHidden.disabled = true;
            } else {
                slugManual.disabled = true;
            }
        });
        ...
        /* ─────────────────────────────────────────────────────
           11. MODAL CONFIRMAR ENVÍO A REVISIÓN
        ───────────────────────────────────────────────────── */
